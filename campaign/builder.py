...
def campaign_from_table(data: List[Dict], istd: str | None = None) -> None:
    ...
    if istd is not None:
        if istd not in [d['compound_name'] for d in data]:
            raise Exception(
                "Cannot find reference chromatogram for internal standard!")

    def parse_data(row: Dict[str, Any]) -> Tuple[str | None, float | None, float | None]:
        ...
        if row["compound_conc"] != "" and row["compound_conc"] is not None:
            try:
                ...
                raise Exception(
                    "Cannot convert Compound Concentration to a number!")
        ...
        if row["istd_conc"] != "" and row["istd_conc"] is not None:
            try:
                ...
                raise Exception(
                    "Cannot convert ISTD Concentration to a number!")
    ...
    for row in data:
        ...
        if idx is not None:
            ...
            if istd != "" and istd is not None and row["compound_name"] == istd:
                camp.istd_chromatogram = row["chromatogram_id"]
    ...
    for row in data:
        ...
        istd_reference = istd != "" and istd is not None and istd == compound_name
